...
async function main() {
    ...
    for(let id in mapping) {
        ...
        let isSolidDarkBg = tlA > 250 && (tlR < 50 && tlG < 50 && tlB < 50);
        ...
        if (isSolidWhiteBg) {
            img.scan(0, 0, img.bitmap.width, img.bitmap.height, function(x, y, idx) {
                ...
                this.bitmap.data[idx + 3] = 255 - v;
            });
            console.log(`Removed white bg for ${mapping[id]}`);
            ...
            console.log(`Removed dark bg for ${mapping[id]}`);
            await img.write(destFile);
        } else {
            // ALREADY TRANSPARENT OR COLORED BACKGROUND (LIKE NYU)
            // Just copy original untouched! CSS brightness(0) invert(1) will make it pure white perfectly!
